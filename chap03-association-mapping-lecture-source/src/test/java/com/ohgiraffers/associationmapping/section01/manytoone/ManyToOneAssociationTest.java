...
public class ManyToOneAssociationTest {
    ...
    *   [다중성에 의한 분류]
    *   : 연관관계가 있는 객체 관계에서 실제로 연관을 가지는 개체의 수에 따라서 분류된다.
    *   - 1:1 (OneToOne)
    *   - 1:N (OneToMany)
    *   - N:1 (ManyToOne)
    *   - N:N (ManyToMany)
    ...
    *   객체 간의 연관관꼐를 양방향으로 만들고 싶은 경우 반대 쪽에서도 필드를
    *   추가해서 참조를 보관하면 된다.
    *   엄밀하게 말해서는 양방향이 아닌 단방향 관계 2개로 볼 수 있다.
    *  */
    ...
    *   이 떄, Menu 엔티티는 Many, Category 엔티티는 One이 된다.
    *   연관 관계를 가지는 엔티티를 조회하는 방법은
    *   1. 객체 그래프 탐색(객체 연관 관계를 사용한 조회)(참조 연산자 . 을 통해 접근 할 수 있다.)
    *   2. 객체 지향 쿼리(JPQL)
    *  */
    ...
    /* 필기.
    *   commit()을 할 경우에는 컨텍스트 내에 저장 된 영속성 객체를 insert 하는 쿼리문이 동작을 한다.
    *   단, 카테고리가 존재하는 값이 아니기 떄문에 부모 테이블(tbl_category) 에 먼저 값이 들어있어야
    *   그 카테고리 코드를 참조하는 자식 테이블에 데이터를 넣을수 있게 된다.
    *   이 때 필요한 것이 @ManyToOne 어노테이션에 연속성 전이 서ㄹ정을 해주는 것이다.
    *   cascade = CascadeType.PERSIST 를 설정하게 된다면
    *   menu_and_category 엔티티를 영속화 할때 Category 엔티티도 함께 영속화를 진행해준다.
    *   */
