...
public class ProjectionTest {
    ...
    *   2. 임베디드 타입 프로젝션
    *   - @Embeddable 엔티티와 거의 비슷하게 사용되며 조회의 시작점이 될수 없다 (FROM 절에 사용 불가)
    *   - 임베디드 타입은 영속 컨텍스트에서 관리하지 않는다.
    *   3. 스칼라 타입 프로젝션
    *   - 숫자, 문자, 날짜 등등의 기본 데이터 타입을 의미한다.
    *   - 스칼라 타입도 영속성 컨텍스트에서 관리되지 않는다.
    *   4. new 명령어를 활용한 프로젝션
    *   - 다양한 종류의 단순 값들을 DTO 로 바로 조회하는 방식으로 'new 패키지명.DTO명' 을 쓰게되면
    *     해당하는 DTO 로 바로 값을 반환 받을 수 있다
    *     역시 이 친구도 클래스 객체는 엔티티가 아니기 떄문에 영속성 컨텍스트에서 관리되지 않는다.
    *    */
    ...
    void testEmbeddedProjection(){
        ...
        /* 조회하는데 embedded 되어있는 menuName 과 menuPrice 조회가 되는지 테스트 */
    ...
    void testNewProjection(){
        ...
        List<CategoryInfoDTO> categoryInfo = repository.newProjection();
