...
public class SimpleJPQLTests {
    ...
    *       ORDER BY 절)
    *   삽입
    *    EntityManager 가 제공하는 persist() 메소드를 사용하면 되므로 따로 없음.
    *   수정
    *       UPDATE
    *       SET
    *       (WHERE)
    ...
    *    1) TypedQuery : 반환할 타입을 명확하게 지정하는 방식일 떄 사용하며 쿼리 객체의 메소드
    *                      실행결과로 지정한 타입이 반환된다.
    *    2) Query : 반환할 타입을 명확하게 지정할 수 없을 떄 사용하며 쿼리 객체 메소드의 실행
    *                 결과로 Object 또는 Object[] 이 반환된다.
    *   3. 쿼리 객체에서 제공하는 메소드 getSingleResult() 또는 getResultList()를 호출해서
    *      쿼리를 실행하고, 데이터베이스를 조회한다.
    *    1) getSingleResult() : 결과가 정확히 한 행인 경우 사용하며, 없거나 많으면 에외 발생
    *    2) getResultList() : 결과가 2개 행 이상인 경우 사용하며, 컬렉션을 반환한다. 없으면 빈 컬렉션 반환
    *  */
...
//        menuList.forEach(menu -> System.out.println(menu));
    ...
    @DisplayName("DISTINCT 를 활용한 중복 제거 다중ㅌ행 조회 테스트")
    @Test
    void testDistinct(){
